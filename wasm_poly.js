...
Module.STDWEB_PRIVATE.acquire_tmp = function( dummy ) {
    ...
    function __instantiate( instance ) {
        ...
        Module.exports.get_clip = function get_clip(poly, clipper) {
    return Module.STDWEB_PRIVATE.acquire_tmp(Module.instance.exports.get_clip(Module.STDWEB_PRIVATE.prepare_any_arg(poly), Module.STDWEB_PRIVATE.prepare_any_arg(clipper)));
}
;
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'wasm_poly'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'wasm_poly':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" && typeof process === "object" ) {
        ...
        const wasm_path = path.join( __dirname, "wasm_poly.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "wasm_poly.wasm" )
